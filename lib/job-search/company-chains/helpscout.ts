...
const COMPANY_NAME = 'HelpScout'
const BASE_URL = 'https://www.helpscout.com'
const JOBS_URL = 'https://www.helpscout.com/company/careers/'

export const companyChain = async (): Promise<CompanyData> => {
  try {
    const html = await urlHTMLReader(JOBS_URL, doc => {
      const header = doc.querySelector('#open-roles')
      const parent = header && header.parentElement
      const jobs = parent && parent.nextElementSibling
